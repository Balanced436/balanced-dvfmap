...
export class HttpClientODS {
  ...
  createUrl(datasetID: string): string {
    let urlCreated =
      this.openDataSoftUrl + datasetID + this.type + this.exportFormat;
    return urlCreated;
  }

  getEPCI() {
    return this.http.get<[]>(this.createUrl(this.epciDatasetID), {
      params: this.params,
    });
  }
  ...
  getCommunes() {
    ...
    return this.http.get<[]>(this.createUrl(this.communeDatasetID), {
      params: this.params,
    });
  }
