...
export class OsmMapComponent implements OnInit, OnDestroy {
  ...
  @Input() options: MapOptions = {
    ...
    zoom: 10,
    center: latLng(
      latLng(CONFIG.localisationReunion[0], CONFIG.localisationReunion[1]),
    ),
  };
  constructor(
    private http: HttpClientODS,
    private postresql: PgsqlBack,
  ...
  setActiveLayer(layername: string) {
    ...
    if (this.activeLayerName == layername) {
      ...
      if (this.choroplethView != 'None' && layername != 'departement') {
         ...
         */
        this.activeLayer = geoJson(this.myLayers[layername].toGeoJSON(), {
        ...
        this.activeLayer.getLayers().forEach((layer) => {
          layer.on('mouseover', (e) => {
            ...
            let nbAppartementsVendues =
              e.target.feature.properties.stats[0].nombre_vente_appartements;
            let prixMedianMaisons =
              e.target.feature.properties.stats[0].prix_m2_median_maisons;
            let prixMedianAppartements =
              e.target.feature.properties.stats[0].prix_m2_median_appartements;
  ...
  findHandler(layername: string): (_e: LeafletMouseEvent) => void {
    let handler: (_e: LeafletMouseEvent) => void = (
